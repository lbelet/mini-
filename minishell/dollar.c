...
int ft_controle(char *str, char **envp, int start, int len)
{
	char *tmp_bis;
	int k;
	char *tmp_cmp;
...
//	tmp[0] = '\0';
	while (str[i])
	{
		...
		else if(str[i] == '$' && code_caractere[i] != 8 && str[i + 1] != '?')
		{
			...
			printf("ici 2\n");
			i--;
			if (ft_controle(str, envp, start, len) == 1)
			{
			...
			else if (ft_controle(str, g_nos_variables, start, len) == 1)
			{
				change = ft_change_value(str, g_nos_variables, start, len);
				printf("ici 4\n");
				tmp = ft_strjoin_free(tmp, change);
				free (change);
			}
